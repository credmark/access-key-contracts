...
describe("Staked Credmark", () => {
  ...
  beforeEach(async () => {
    ...
    [wallet, secondWallet, thirdWallet, fourthWallet] = await ethers.getSigners();
  });
  ...
  describe("#createShare", () => {
    ...
    it("should not create share if low cmk balance", async () => {
      await cmk.connect(secondWallet).approve(stakedCmk.connect(secondWallet).address, 100);
      await expect(stakedCmk.connect(secondWallet).createShare(100)).to.be.reverted;
    });
  });
  ...
  describe("#complexMath", () => {
    it("should have perfect CMK->xCMK and xCMK->CMK conversion for multiple wallets", async () => {
      ...
      expect(await stakedCmk.sharesToCmk(1_000_000)).to.be.equal(BigNumber.from(1_000_000));

      await stakedCmk.connect(secondWallet).createShare(500_000);
      expect(await stakedCmk.balanceOf(secondWallet.address)).to.be.equal(BigNumber.from(500_000));
      expect(await stakedCmk.cmkBalanceOf(secondWallet.address)).to.be.equal(BigNumber.from(500_000));
      expect(await stakedCmk.sharesToCmk(1_000_000)).to.be.equal(BigNumber.from(1_000_000));

      await stakedCmk.connect(thirdWallet).createShare(400_000);
      expect(await stakedCmk.balanceOf(thirdWallet.address)).to.be.equal(BigNumber.from(400_000));
      expect(await stakedCmk.cmkBalanceOf(thirdWallet.address)).to.be.equal(BigNumber.from(400_000));
      expect(await stakedCmk.sharesToCmk(1_000_000)).to.be.equal(BigNumber.from(1_000_000));

      await stakedCmk.connect(fourthWallet).createShare(100_000);
      expect(await stakedCmk.balanceOf(fourthWallet.address)).to.be.equal(BigNumber.from(100_000));
      expect(await stakedCmk.cmkBalanceOf(fourthWallet.address)).to.be.equal(BigNumber.from(100_000));
      expect(await stakedCmk.sharesToCmk(1_000_000)).to.be.equal(BigNumber.from(1_000_000));
      ...
      expect(await stakedCmk.cmkBalanceOf(fourthWallet.address)).to.be.equal(BigNumber.from(100_000).add(5_000)); // 100_000 * (100_000 / 2_000_000)
      ...
      expect(await stakedCmk.cmkBalanceOf(fourthWallet.address)).to.be.equal(BigNumber.from(189_000).add(101_123)); // 1_000_000 * (180_000 / 1_780_000)
      ...
      expect(await stakedCmk.cmkBalanceOf(wallet.address)).to.be.equal(BigNumber.from(0));
      ...
      await expect(stakedCmk.connect(secondWallet).removeShare(100_000))
        .to.emit(cmk, "Transfer")
        .withArgs(stakedCmk.address, secondWallet.address, BigNumber.from(161_179)); // 100_000 * (2_063_102 / 1_280_000)
      expect(await stakedCmk.cmkBalanceOf(secondWallet.address)).to.be.equal(BigNumber.from(0));
      ...
      await expect(stakedCmk.connect(thirdWallet).removeShare(1_000_000))
        .to.emit(cmk, "Transfer")
        .withArgs(stakedCmk.address, thirdWallet.address, BigNumber.from(1_611_799)); // 1_000_000 * (1_901_923 / 1_180_000)
      expect(await stakedCmk.cmkBalanceOf(thirdWallet.address)).to.be.equal(BigNumber.from(0));
      ...
      await expect(stakedCmk.connect(fourthWallet).removeShare(180_000))
        .to.emit(cmk, "Transfer")
        .withArgs(stakedCmk.address, fourthWallet.address, BigNumber.from(290_124)); // 1_000_000 * (1_901_923 / 1_180_000)
      expect(await stakedCmk.cmkBalanceOf(fourthWallet.address)).to.be.equal(BigNumber.from(0));
