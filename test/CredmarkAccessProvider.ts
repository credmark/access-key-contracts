...
describe("Credmark Access Provider", () => {
  ...
  beforeEach(async () => {
    ...
    [cmk, credmarkAccessKey, credmarkAccessProvider] = await waffle.loadFixture(fixture);
  });
  ...
  it("should authorize token owner", async () => {
    ...
    expect(await credmarkAccessProvider.authorize(wallet.address, tokenId)).to.be.equal(true);
  });
  ...
  it("should not authorize other than token owner", async () => {
    ...
    expect(await credmarkAccessProvider.authorize(otherWallet.address, tokenId)).to.be.equal(false);
  });
