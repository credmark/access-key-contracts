...
import { MockCMK, StakedCredmark, RewardsPool } from '../typechain';
...
describe('Rewards Pool', () => {
  ...
  const fixture = async (): Promise<[MockCMK, StakedCredmark, RewardsPool]> => {
    ...
    const _rewardsPool = (await rewardsPoolFactory.deploy(
      _cmk.address,
      _stakedCmk.address
    )) as RewardsPool;
  ...
  describe('#start', () => {
    it('should not be started by non owner', async () => {
      ...
      await expect(otherRewardsPool.start(0)).to.be.revertedWith(
    ...
    it('should not be started with end time in past', async () => {
      await expect(
        rewardsPool.start(BigNumber.from(Date.now()).div(1000).sub(sevenDays))
      ).to.be.revertedWith('End time is not in future');
    });
    ...
    it('should not restart', async () => {
      await rewardsPool.start(now.add(sevenDays));
      await expect(rewardsPool.start(now.add(sevenDays))).to.be.revertedWith(
  ...
  describe('#endTime', () => {
    ...
    it('should not allow non-owner to set end time', async () => {
      ...
      const otherRewardsPool = rewardsPool.connect(otherWallet);
      await expect(
        otherRewardsPool.setEndTime(now.add(sevenDays))
    ...
    it('should not allow to set end time in the past', async () => {
      await rewardsPool.start(now.add(sevenDays));
      await expect(
        rewardsPool.setEndTime(
          BigNumber.from(Date.now()).div(1000).sub(sevenDays)
        )
      ).to.be.revertedWith('End time is not in future');
    });
  });
  ...
  describe('#unissuedRewards', () => {
    ...
    it('should increase unissued rewards with time', async () => {
      ...
      expect(unissuedRewardsAfter.sub(unissuedRewardsBefore)).to.be.closeTo(
  ...
  describe('#issueRewards', () => {
    it('should issue rewards to staked cmk', async () => {
      ...
      await expect(rewardsPool.issueRewards()).to.emit(
        rewardsPool,
        'RewardsIssued'
      );
      expect(await cmk.balanceOf(rewardsPool.address)).to.be.closeTo(
        BigNumber.from(10000000).sub(unissuedRewards),
        100
      );
      expect(await cmk.balanceOf(stakedCmk.address)).to.be.closeTo(
        unissuedRewards,
        100
      );
    });
    ...
    it('should issue rewards proportional to time passed', async () => {
      ...
      expect(await cmk.balanceOf(rewardsPool.address)).to.be.equal(
        BigNumber.from(0)
      );
      expect(await cmk.balanceOf(stakedCmk.address)).to.be.equal(
    ...
    it('should issue rewards on updating end time', async () => {
      ...
      await expect(
        rewardsPool.setEndTime(now.add(sevenDays).add(sevenDays * 2))
      ).to.emit(rewardsPool, 'RewardsIssued');
